...
!        real(8), intent(in) :: t0, tf, dt0, omega, jac
         real(8), intent(in) :: t0, tf, dt0, jac
...
!        real(8) :: err_psi(nrun-1), err_phi(nrun-1)
         real(8) :: err_wf(2,nrun-1)
         integer :: k
      
!        allocate(psi(nmax,nrun), phi(nmax,nrun))
         allocate(wf(nmax,nchan+2,nrun))
...
!                                       psi(:,k), phi(:,k),           &
         ...
         ! --- errors ---
         do k = 1, nrun-1
            err_wf(1,k) = sqrt(sum(abs(wf(:,1,k) - wf(:,1,k+1))**2))
            err_wf(2,k) = sqrt(sum(abs(wf(:,2,k) - wf(:,2,k+1))**2))
         end do
      
         ! --- observed order (last two refinements) ---
         order_psi = log(err_wf(1,nrun-2)/err_wf(1,nrun-1)) / log(2.0d0)
         order_phi = log(err_wf(2,nrun-2)/err_wf(2,nrun-1)) / log(2.0d0)
...
!         complex(8), allocatable :: wf_exact(nmax,nchan+2), svec(:,:)
!         real(8) :: errL2_psi(nrun), errL2_phi(nrun)
!         real(8) :: errMax_psi(nrun), errMax_phi(nrun)
!!        real(8) :: norm_psi(nrun), norm_phi(nrun)
!         real(8) :: norm_wf(nchan+2,nrun)
!!        real(8) :: rel_psi(nrun), rel_phi(nrun)
!         real(8) :: rel_psi(nchan+2,nrun)
!         real(8) :: order_psi, order_phi
!
!         integer :: k
...
!  *** One loop full run drivers 
      ...
      subroutine run_split_operator_driver( nmax, nchan,              &
                               xx, dt, tf, t0,                        &
                               eigval, eigvec,                        & 
!                              psi0, phi0,                            &
!                              psi, phi, order )
...
!        phi_in = phi0
         ...
         do i = 1, nt
            ...
            call split_operator( nmax, nchan,                         &
                                      dt, t, xx, eigval, eigvec,      &
                                      wf_in, wf, order )
...
!           phi_in = phi
            ...
            if (mod(i,10) == 0) then
               write(*,'(f8.4,3x,6e16.8)') t,                       &
                    real(wf(1,1)), aimag(wf(1,1)),                    &
                                         sqrt(sum(abs(wf(:,1))**2)),    &
                    real(wf(2,2)), aimag(wf(2,2)),                    &
                                         sqrt(sum(abs(wf(:,2))**2))
            end if
...
!        complex(8), dimension(nmax) :: psi_in, phi_in, src
         complex(8), dimension(nmax, nchan+2) :: wf_in, src
...
!      write(*,'(a)') '         Inside of convergence test         '
       ...
       write(log_unit,'(a)') '# t  Re[psi(1)] Im[psi(1)] ||psi||  '// & 
             ...
             write(log_unit,'(f10.4,3x,6e16.8)') t,                 &
                  real(wf_in(1,1)), aimag(wf_in(1,1)),              &
                                     sqrt(sum(abs(wf(:,1))**2)),    &
                  real(wf_in(j,2)), aimag(wf_in(j,2)),              &
                                       sqrt(sum(abs(wf_in(:,2))**2))
...
!        call split_operator(nmax, dt, t, xx, eigval, eigvec,         &
!                                                  psi_in, psi, order)
         call split_operator(nmax, nchan, dt, t, xx, eigval, eigvec,   &
                                                   wf_in, wf, order)
